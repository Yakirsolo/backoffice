...
Everything runs on a single EC2 instance (`t4g.small`, Graviton/arm64, `il-central-1`) via Docker Compose — three containers (`postgres`, `backend`, `frontend`) on one Docker network, no RDS, no load balancer. `frontend`'s nginx is the only container that binds a host port (`80:80`); it serves the built Angular app and reverse-proxies `/api/` to `backend:8080` inside the compose network (`frontend/nginx.conf`), so `backend` is never exposed to the internet directly.

- **Networking**: security group `backoffice-sg` allows inbound 80 and 443 from anywhere, 22 (SSH) from a single admin IP only. **No Elastic IP is allocated** — the instance's public IP is ephemeral and changes if the instance is ever stopped/started, which also breaks `CORS_ALLOWED_ORIGIN` (baked in fresh on every deploy — see below) until the next deploy re-runs. 443 is open at the security-group level but nginx doesn't listen on it — no TLS is terminated anywhere in this stack yet.
- **Secrets/config**: real secrets (`db_password`, `jwt_secret`, `storage_secret_key`, `storage_access_key_id`, `admin_password`) live in SSM Parameter Store under `/backoffice/*` as `SecureString`s — never in the repo or in GitHub Actions secrets. `/home/ec2-user/app/fetch-env.sh` on the instance pulls all of them, plus the instance's own public IP from the EC2 metadata endpoint, into a `.env` file that `docker compose` reads. That script runs on every deploy, right before `docker compose pull && up -d`.
- **File storage is AWS S3, not R2** — production's `.env` sets `STORAGE_PROVIDER=s3` against bucket `backoffice-<account-id>-storage` (`il-central-1`), via a dedicated IAM user (`backoffice-app-storage`) scoped to only `s3:PutObject`/`s3:GetObject` on that bucket's objects. `StorageService` itself is provider-agnostic and would work against Cloudflare R2 too (`STORAGE_PROVIDER` defaults to `r2` when unset) — S3 is a deployment choice, not a code constraint.
...
Known gaps worth checking before assuming otherwise: no Elastic IP (allocate one before pointing any DNS/domain at the instance), no TLS termination in `nginx.conf` (needed for the refresh-token cookie's `Secure`+`SameSite=None` flags to actually work in a browser — see `AuthController`), no RDS backups.
